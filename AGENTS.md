...
- `npm test` = full suite. Focused: `node --test test/<area>-*.test.ts` (areas: `daemon-`, `runner-`, `cli-`, plus `gate`, `e2e-delegate`, `harness-mirrors`, `cloud-agnostic`, `infra-aws`, `packaging`, `sanitized`, `analyzer-scope`, `complexity`, `temporal-tokens`). Optional: `FLEET_DEMO_HISTORY=<path> npm test` round-trips an external history file. `FLEET_HARNESS_CORPUS=<path> npm test` replays a captured real harness stream through the translator. Both point outside the repo — external data is never vendored (`docs/decisions.md#d10`).
- **`pip install lizard` before you push.** Every Codacy issue this repo's code has ever had came from Lizard (a function over 50 lines or complexity 10), and Codacy only reports it on a pull request — so without lizard installed you find out by pushing and waiting. `test/complexity.test.ts` runs it against a pinned baseline of the functions already over the line (paying one off means deleting its entry — the gate fails until the debt list matches reality); it skips silently when lizard is absent, which makes it a check you can accidentally not have.
- **`brew install chafa` if you touch the banner art.** The CLI's paper airplane is baked (`src/cli/banner-art.ts`), generated from `fixtures/dart.png` by `node fixtures/bake-banner-art.ts`; `test/cli-banner-art.test.ts` re-runs chafa and fails on drift. Like lizard, it skips when the tool is absent — a check you can accidentally not have. chafa is a build-harness tool: nothing invokes it at runtime and it never ships.
...
| `test/` `fixtures/` | BUILD | The suite (incl. sanitization, mirror-drift, cloud-agnostic, packaging gates); executable fixtures + synthetic data; the history⇄events converter (`test/history-events.mjs`) backing the `FLEET_DEMO_HISTORY` round-trip; the banner-art source and its generator (`fixtures/dart.png`, `fixtures/bake-banner-art.ts` → `src/cli/banner-art.ts`) |
