...
test('an override plus a prompt says the prompt was dropped, rather than dropping it quietly (#240)', () => {
  // The one combination where --prompt does nothing: the override already is
  // the launch line. Fleet cannot splice the prompt into it without putting
  // operator text back in a shell string (#241), so it drops it — and the
  // failure mode worth a test is an operator watching a job ignore the
  // instruction they typed with nothing in the log to explain it.
  const plan = buildHarnessCommand({
    manifest,
    target: 'APP-14',
    override: 'node fake.mjs',
    prompt: '/dev-sprint',
  });
  assert.ok(plan);
  assert.equal(plan.args.length, 0, 'the override is verbatim; the prompt must not become an argument to it');
  assert.match(plan.notes.join('\n'), /FLEET_HARNESS_CMD/, `the drop was silent: ${JSON.stringify(plan.notes)}`);
  assert.match(plan.notes.join('\n'), /prompt was not used/, `the note does not say what was dropped: ${JSON.stringify(plan.notes)}`);
  // And the same override without a prompt has nothing to warn about.
  const quiet = buildHarnessCommand({ manifest, target: 'APP-14', override: 'node fake.mjs' });
