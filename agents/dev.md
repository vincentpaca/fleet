...
1. **Repo lane** — the job branch and optional draft PR. The lane when the deliverable is a code change; also for docs and ADRs that should be versioned. Requires `authority.publish` in the order, which a dispatch against an issue or a PR gets by default and a prose dispatch gets only with `--publish` (#36).
