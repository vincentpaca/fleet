...
6. **Prove your work is on the branch.** `git log origin/$(git branch --show-current)..HEAD --oneline` in the workspace must list your commits, and `git status` must be clean after committing. If your harness ran subagents in isolated worktrees, their edits are NOT in this workspace until applied and committed here — a clean workspace means you are about to deliver nothing, whatever your summary says (this exact failure burned #34's first run).
7. **Report.** Write `.fleet/out/report.json`: `status` (`READY` if all acceptance criteria are met and the suite is green, `PARTIAL` otherwise), `verification` (an **array** of the exact commands you ran — a bare string is schema-rejected and your report is dropped), `not_done` (anything remaining), and exactly one `next_action`.
