---
description: Implement a GitHub issue on this repo, headless. Reads the issue, honors its acceptance criteria, raises human decisions via the decision file, self-reviews with the code-reviewer agent, and writes a status-first report before finishing.
---

Implement GitHub issue #$ARGUMENTS on this repository.

The pickup gate has already verified the issue is open, labeled `ready`, has an `## Acceptance` section, and no branch claims it. Repo conventions are in AGENTS.md — the hard rules there are binding.

## Steps

1. **Read the work.** `gh issue view $ARGUMENTS --json title,body,comments`. The `## Acceptance` section is the contract; the referenced `docs/` files are the context. If the issue references docs anchors, read those sections.
...
3. **Implement.** Follow AGENTS.md rules (erasable TS, zero deps, schemas are source of truth). If working in a fleet job workspace, stay on the current branch — it is the job branch.
...
5. **Self-review.** Invoke the `code-reviewer` agent on your diff. Fix warranted findings; do not resolve a finding without a fix or a stated reason.
