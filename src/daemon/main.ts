...
import { EcsProvider, ecsConfigFromEnv } from "../providers/ecs.ts";
import type { Provider } from "../providers/provider.ts";

function pickProvider(name: string): Provider {
  switch (name) {
    case "process":
...
const daemon = new FleetDaemon({
  home,
  provider: pickProvider(process.env.FLEET_PROVIDER ?? "process"),
  port,
});
