...
- **CLI** (`src/cli/`): thin. Sends requests, renders events, exits. `delegate` is one HTTP call; nothing keeps running on your machine unless you ask it to watch. Bare `fleet` on a terminal is the **cockpit** (`src/cli/cockpit.ts`): the live board, the selected job's tail, and a command line, holding the daemon tunnel for as long as it is open — one resident surface over the same event stream and answer API every other command uses. It is a view, not a component: it adds no endpoint, no state, and nothing a job depends on.
...
- **Infra units** (`infra/<cloud>/`): one self-contained Terraform module per cloud — each satisfies the same requirements (on-demand container execution, daemon hosting with durable state, image registry, operator access with no public ingress, scale to zero behind a hard capacity cap) its own way, and pairs with a `src/providers/` implementation. `infra/aws/` is the first unit: ECS cluster + capacity provider, ECR, IAM, EFS-backed daemon service, SSM-only access. `infra/gcp/` is the second (#185, `docs/decisions.md#d18`): jobs are Cloud Run Job executions (scale-to-zero and per-second billing are inherent, so it ships no ASG and no warm floor), the daemon is a micro VM with a reserved internal address and a separate data disk as `FLEET_HOME` — installed from npm at a pinned version by cloud-init, there is no GCP daemon image — and operator access is an IAP tunnel to a VM with no external IP. One apply per account, and the CLI owns it: `fleet setup infra` (`src/cli/setup.ts`, unit map in `src/cli/setup-units.ts`) interviews for the values a unit's defaults cannot assume, generates `.fleet/infra/<provider>/main.tf` against the unit at a pinned git ref, plans, applies on an explicit yes, and captures `fleet_config` — the file every other command resolves the deployment from. Because Fleet authors the infra shape, that interview is short by construction; a second cloud is an entry in the unit map, not a branch in the wizard. Units provision no billing products — spend is bounded structurally, and billing alarms belong to the operator (`docs/decisions.md#d12`).
...
On a **GCP** deployment the same script publishes `:runner` alone through `gcloud auth configure-docker`, and its roll step is a printed note: that cloud has no daemon image (`docs/decisions.md#d18`). Either way this is the developer/offline path — the supported one is the wizard's in-account build (#189): the infra unit provisions a one-shot build pinned to the same git ref as the module itself (CodeBuild on AWS, Cloud Build on GCP), `fleet setup infra` starts it after the apply and waits with progress, and `--rebuild-images` (also driven by `fleet upgrade`) re-runs it. No shipped code path deploys anything afterwards — the unit's own `rollHint` says what the operator must still do, or that there is nothing to do (`docs/decisions.md#d5`).
...
**Setup privilege contract (#196):** trust follows authorship. `setup.script` is operator-written and manifest-declared (laptop trust class), so it runs as root and can install system packages; the harness runs untrusted job content, so it never does. The runner container starts as root, keeps root exactly as long as setup needs it, then irreversibly drops to the unprivileged job user (uid 1000, `dropPrivileges` in `src/runner/setup.ts`) — the pickup gate, the harness, and the settle all run unprivileged. A manifest with no `setup.script` drops immediately, before the clone. The two-layer path honors the same split: the baked setup layer runs as root during `docker build`, and the container still drops before any job work. A drop that cannot complete cancels the job with reason `privilege-drop` rather than run the agent as root.
...
Wall-clock is a cost bound, not a liveness check: a job that stalls silently still burns the whole budget before anything notices (one job spent 3.5 hours wedged on an API rate-limit stall, diagnosable after fifteen minutes of silence). So the runner also meters **liveness**, and the signal is the harness *process*, not its stdout: a live process that prints nothing is normal (a long tool call, a backgrounded command), and one live job was killed while actively finishing for exactly that silence. While the process runs, the runner emits one bounded "harness working" line per third of `limits.idle` (default 20m) on a timer — whether the harness floods it with untranslatable noise or goes completely quiet — which is what feeds the daemon's backstop: a running job whose last *event* is older than `idle` plus a margin means the runner itself is dead or wedged, and the daemon terminates the container, then waits out the runner's teardown deadline before synthesising a settle, so a teardown that is still delivering (WIP push, artifacts, its own settle) is recorded rather than 422-rejected mid-sentence. The runner's own idle clock (`limits.idle`, blocked time excluded) still cancels with reason `stall`, partial work pushed — it now catches the wedge process liveness cannot vouch for, a harness whose process died leaving a pipe-holding child. Blocked jobs are exempt on both sides; waiting on a human is not a stall, and the hot→park→stale lifecycle already owns that wait. `fleet status` and the cockpit render terminal jobs as `cancelled(<reason>)`, so `cancelled(stall)` is distinguishable from `cancelled(wall-clock)` at a glance.

Delivery does not wait for the end, either: every `limits.checkpoint` (default 10m) the runner commits and pushes WIP to the job branch, so a container killed at any moment — the wall-clock cliff, a stall cancel whose final push then fails, a plain SIGKILL — loses at most one interval of work instead of all of it. Checkpoint pushes ride the same bounded push machinery as the teardown's; a failure logs and the run continues, and the final push moves an incremental delta instead of the whole diff.
...
- Operator endpoints trust the unix socket's permissions locally, and the operator's own AWS identity via SSM port-forwarding remotely. Zero open ports. `fleet connect` — and the cockpit, which holds one the same way while it is open, adopting a healthy tunnel rather than fighting it for the port — owns that forward rather than leaving it to hand-run cloud commands: it resolves the deployment from `.fleet/infra/<provider>/fleet-config.json` (falling back to the SSM parameter that file names), opens the forward, verifies `/health`, and reopens on session death with backoff — re-resolving the daemon's container every time, because a service deployment replaces it. A live session is not left silent: `/health` is polled through the forward every two minutes, well inside SSM's idle timeout, because a session that carries no traffic is reaped for it — and that poll doubles as death detection for a forward that outlives what it points at, which no process exit would ever report. Nothing about the tunnel is a privilege: it carries the operator's own identity, and a dead one costs availability, never authority. `fleet doctor` reports its state (port open, `/health` answering, endpoint still current) so a dead session surfaces as itself instead of as `ECONNREFUSED` at the next dispatch. Cloud-specific mechanics live in the cloud's own unit; `src/cli/tunnel-openers.ts` is the one place in core that maps a deployment's `provider` to them.
