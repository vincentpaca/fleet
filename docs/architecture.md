...
Wall-clock is a cost bound, not a liveness check: a job that stalls silently still burns the whole budget before anything notices (one job spent 3.5 hours wedged on an API rate-limit stall, diagnosable after fifteen minutes of silence). So the runner also meters **silence**: no harness output for `limits.idle` (default 20m, blocked time excluded, same exclusion as the budget) means SIGTERM, partial work pushed, and a settle whose report carries the idle duration, cancelled with reason `stall`. Idle detection is always armed — unlike a spend budget, silence is never an intended state — and the daemon mirrors it as a backstop for a runner that cannot report on itself: a running job whose last event is older than `idle` plus a margin gets the same treatment. Blocked jobs are exempt on both sides; waiting on a human is not a stall, and the hot→park→stale lifecycle already owns that wait. `fleet status` and the cockpit render terminal jobs as `cancelled(<reason>)`, so `cancelled(stall)` is distinguishable from `cancelled(wall-clock)` at a glance. The two clocks measure different streams — the runner watches the harness's stdout, the daemon watches the event log — and that gap is real: the translator drops the harness's own progress heartbeats as noise, so a job inside one long tool call is loud on stdout and silent on events. The runner closes it by coalescing those dropped lines into one bounded liveness line per third of the idle limit; the backstop stays a backstop rather than a second opinion that kills live work it cannot push.
