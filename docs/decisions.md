...
**Two are open questions, not decisions.** The daemon image still runs as root: it writes to the EFS mount, and dropping to uid 1000 needs either an access point that relocates the mount or a `chown` of a live filesystem — an operator action against existing data, not a code change. The worker launch template keeps `http_put_response_hop_limit = 2`, because the line next to it says containers need the instance role through IMDS; 1 is the safer number and the escalation it prevents is exactly the one Fleet's permission split exists for, so if nothing in a job actually reads IMDS this should change. Both want an issue and a human, not a quiet edit.
