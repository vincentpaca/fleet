...
Per job, that snapshot is two files. `job.json` is the hot record, rewritten once per event; `launch.json` holds the write-once launch data — the manifest, the synced-file map, the work order — which is most of the bytes and none of the change. Keeping them together made every event re-serialise a manifest, which on EFS is a per-event network round trip sized by data that never moves. A job created before the split has no `launch.json` and the loader falls back to the fields still inside its `job.json`, so there is no migration step. An unreadable `launch.json` is logged, not fatal: it is only needed to re-launch a parked job, and a job whose journal is intact still serves its history and still settles.
