...
1. Your repo describes its environment in `.fleet/manifest.json`: base image or devcontainer, setup script, which gitignored config files to copy in, which env vars and services the job needs, and which coding agent drives each job. `fleet setup repo` writes it by interview, with the defaults read out of your checkout — and each setup ends by offering the next one, so bare `fleet setup` walks the whole path.
...
3. `fleet delegate` builds the sandbox, runs your repo's readiness gate (a script you own; if it fails, the job stops before any model spend), then runs **what you typed** headless and streams progress events back. A dispatch is a target and a prompt: prose runs as typed and delivers a report, while an issue or PR target takes `--prompt` — `fleet delegate 42 --prompt "/dev-work #42"` — and delivers a draft PR. A bare issue number is refused before a container boots: it names which work, never what to do about it.
...
fleet delegate 42 --prompt "/dev-work #42"    # an issue: ends in a draft PR
